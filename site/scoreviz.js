...
(function (root) {
  ...
  // Portrait: roots go DOWN. Depth is the story, width is just room for labels.
  var W = 560, H = 660;
  var SURFACE = 76;          // y of the soil line
  var FLOOR = 566;           // y of the deepest point
  var CX = 280;              // taproot x
  ...
  function scoreSvg(opts) {
    ...
    var step = Math.max(34, Math.min(70, tapLen / 3.6));   // rungs stretch with depth
    var attachStep = Math.min(step * 0.75, Math.max(6, (tapLen - 14) / Math.max(1, perSide - 1)));
    for (var q = 0; q < n; q++) {
      ...
      var ye = SURFACE + 44 + k * step;                      // where it ends (the rung) — well below where it left
      var reach = 34 + 104 * (pts / per);                    // 0 points still gets a nub, so the label has a place
      var ex = CX + dir * reach;
      // Leaves the taproot heading down-and-out, and settles onto its rung: depth first.
      var d = 'M' + CX + ' ' + r1(ya)
        + ' C ' + r1(CX + dir * reach * 0.22) + ' ' + r1(ya + (ye - ya) * 0.45)
        + ', ' + r1(CX + dir * reach * 0.62) + ' ' + r1(ya + (ye - ya) * 0.92)
        + ', ' + r1(ex) + ' ' + r1(ye);
